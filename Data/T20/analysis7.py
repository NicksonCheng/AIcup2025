...
output_selected_features_file = "/home/r1419r1419/Stock_Competition/Data/T20/selected_features7.csv"
chunksize = 20000
...
# 選出每種方法的前 20 個特徵並生成結果
results = []
for method, scores in feature_scores.items():
    top_features = pd.Series(scores).sort_values(ascending=False).head(20)
    print(f"{method} 前 20 個特徵: {top_features.index.tolist()}")
...
final_results.to_csv(output_selected_features_file, index=False, encoding='utf-8')  # 移除 line_terminator
