...
model_dir = "/home/r1419r1419/Stock_Competition/Data/T50/Ensemble_model"
...
print(f"載入模型: {lgb_model_path}")

print("載入 XGBoost 模型...")
with open(xgb_model_path, 'rb') as f:
    xgb_model = pickle.load(f)
print(f"載入模型: {xgb_model_path}")
...
lgb_preds_proba = lgb_model.predict(X_test_scaled)
xgb_preds_proba = xgb_model.predict(X_test_dmatrix)
ensemble_preds_proba = 0.5 * lgb_preds_proba + 0.5 * xgb_preds_proba
test_preds = (ensemble_preds_proba > THRESHOLD).astype(int)
