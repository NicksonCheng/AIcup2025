...
output_file = "/home/r1419r1419/Stock_Competition/Data/T50_allNG/selected_training8_SE.csv"
chunksize = 10000

# 目標樣本數
target_positive_samples = 5000  # SMOTE 至 5,000
...
# 4. 使用 Pipeline 執行 SMOTE + ENN
print(f"使用 SMOTE 生成正樣本至 {target_positive_samples} 並應用 ENN...")
smote = SMOTE(sampling_strategy={1: target_positive_samples}, random_state=42, k_neighbors=min(5, n_positive-1))
enn = EditedNearestNeighbours(sampling_strategy='all', n_neighbors=3)  # 清理正負樣本
pipeline = Pipeline([('smote', smote), ('enn', enn)])
X_resampled, y_resampled = pipeline.fit_resample(X_all, y_all)
print(f"SMOTE + ENN 後正樣本數: {sum(y_resampled == 1)}")
print(f"SMOTE + ENN 後負樣本數: {sum(y_resampled == 0)}")
