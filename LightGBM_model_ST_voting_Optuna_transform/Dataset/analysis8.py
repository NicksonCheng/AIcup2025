...
output_selected_features_file = "/home/r1419r1419/Stock_Competition/LightGBM_model_ST_voting_Optuna_transform/Dataset/selected_features8.csv"
chunksize = 15000
...
# 初始化特徵重要性字典（新增 AutoEncoder）
feature_scores = {
    'LightGBM': {}, 'MutualInfo': {}, 'Spearman': {}, 'RandomForest': {},
    'Pearson': {}, 'ChiSquare': {}, 'Lasso': {}, 'AutoEncoder': {}
}
...
for chunk_idx, chunk in enumerate(pd.read_csv(input_file, chunksize=chunksize)):
    ...
    # Auto-Encoder
    X_scaled_ae = StandardScaler().fit_transform(X)  # 標準化數據
    input_dim = X_scaled_ae.shape[1]
    encoding_dim = 32  # 隱層維度，可調整
    input_layer = Input(shape=(input_dim,))
    encoded = Dense(encoding_dim, activation='relu')(input_layer)
    decoded = Dense(input_dim, activation='linear')(encoded)
    autoencoder = Model(input_layer, decoded)
    autoencoder.compile(optimizer='adam', loss='mse')
    autoencoder.fit(X_scaled_ae, X_scaled_ae, epochs=10, batch_size=32, verbose=0)
    feature_importance_ae = np.mean(np.abs(autoencoder.layers[1].get_weights()[0]), axis=1)  # 計算輸入層權重平均
    for f, s in zip(X.columns, feature_importance_ae):
        feature_scores['AutoEncoder'][f] = feature_scores['AutoEncoder'].get(f, 0) + s
...
print(f"總計選出 {len(final_results)} 條記錄（8 方法各 {top_n} 個特徵）")
